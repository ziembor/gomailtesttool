...
- Updated `msgraphtool_integration_test.go` with `TestIntegration_CheckAvailability()`
...
- New CSV schema for getschedule action:
  ...
  - File format: `_msgraphtool_getschedule_YYYY-MM-DD.csv`
...
- Removed unused `strings` import from msgraphtool.go after completion flag refactoring
...
**Important**: This tool requires Exchange Online RBAC permissions, NOT Entra ID API permissions.

**Exchange Online RBAC Permissions**:
- **Application Mail.ReadWrite** - Required for sendmail, getinbox actions
- **Application Calendars.ReadWrite** - Required for getevents, sendinvite, getschedule actions
...
- **Recommended Role**: Exchange Administrator (from PIM)
...
- Permissions are assigned via PowerShell, not through Entra ID portal
...
.\msgraphtool.exe -tenantid "xxx" -clientid "yyy" -secret "zzz" `
