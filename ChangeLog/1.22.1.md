...
### 🔒 CRITICAL: Fixed OData Injection Vulnerability in searchAndExport (CVE-2026-MSGRAPH-001)
...
3. **Added OData Escaping** (`src/handlers.go`)
   ...
   - Single quotes escaped using OData rules: `'` → `''`
   - Provides fallback protection if validation is bypassed

4. **Comprehensive Security Testing** (`src/msgraphtool_test.go`)
...
**EXAMPLES.md**
- ✅ Added Section 17: Export Inbox to JSON
...
- ✅ Added Section 18: Search and Export by Message ID
...
- ✅ Updated Section 20: Environment Variables Reference
  - Added `-messageid` → `MSGRAPHMESSAGEID` mapping
- ✅ Updated version reference: v1.16.0 → v1.22.1

**README.md**
- ✅ Updated operations list to include: "Export Inbox, Search and Export"
- ✅ Comprehensive feature list now complete

**CLAUDE.md** (Internal Documentation)
- ✅ Added "Export Inbox" to Graph Operations section
  - Export inbox messages to individual JSON files
  - Date-stamped directories (`%TEMP%\export\{date}`)
- ✅ Added "Search and Export" to Graph Operations section
...
**TROUBLESHOOTING.md**
- ✅ Updated Permission Errors table
  - Added `exportinbox` → Application Mail.ReadWrite
  - Added `searchandexport` → Application Mail.ReadWrite
- ✅ Added new section: "Export Directory and File Issues"
...
- ✅ Updated version reference to v1.22.1

**ARCHITECTURE_DIAGRAM.md**
- ✅ Updated Action Handlers section
  - Added `exportInbox()` (-action exportinbox)
  - Added `searchAndExport()` (-action searchandexport)
- ✅ Updated Core Graph API Operations section
  - Added exportInbox() operation flow
  - Added searchAndExport() operation flow with filter details
- ✅ Updated CSV Logging Pattern section
  - Added `_msgraphtool_exportinbox_*.csv`
  - Added `_msgraphtool_searchandexport_*.csv`
- ✅ Added new section: "JSON Export Pattern (v1.21.0+)"
  - Documented date-stamped directory structure
  - Example file naming conventions
- ✅ Updated version and date to v1.22.1 / 2026-01-07

### Security Documentation

**SECURITY.md**
- ✅ Added new section: "Security Vulnerabilities Fixed"
- ✅ Documented CVE-2026-MSGRAPH-001: OData Injection vulnerability
...
**IMPROVEMENTS.md**
- ✅ Added Recommendation #9: Fix OData Injection Vulnerability
  - Status: PENDING → COMPLETED in v1.22.1
...
- ✅ Updated Executive Summary
  ...
  - Downgraded overall grade: A → B+ (due to vulnerability)
  - Security issue now resolved in v1.22.1
- ✅ Updated Summary by Priority table
  - Added CRITICAL priority tier
  - Total recommendations: 8 → 9
...
- ✅ Added Update History entry for 2026-01-07 security review
...
- `src/msgraphtool_test.go` - Added security tests (55 lines)
...
Copy-Item msgraphtool.exe msgraphtool.exe.bak
...
# Should show: msgraphtool version 1.22.1

# 4. SECURITY AUDIT: Review logs for suspicious activity
Get-ChildItem "$env:TEMP\_msgraphtool_searchandexport_*.csv" |
    Import-Csv |
...
**For v1.21.0 Users:**
- ⚠️ **CRITICAL:** Upgrade to v1.22.1 immediately
- 📋 **Audit:** Review CSV logs for suspicious Message-ID patterns
- 🔍 **Assess:** Check Microsoft 365 audit logs for unauthorized mailbox access
- 📧 **Report:** Contact security team if suspicious activity detected

**For v1.21.1+ Users:**
- ✅ Security fix already applied
- ℹ️ v1.22.1 adds comprehensive documentation updates
