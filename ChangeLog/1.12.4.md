...
- Project structure reorganized: all Go source files moved to `src/` subdirectory
  - `msgraphtool.go` → `src/msgraphtool.go`
  - `cert_windows.go` → `src/cert_windows.go`
  - `cert_stub.go` → `src/cert_stub.go`
  - `go.mod` and `go.sum` → `src/`
- Build commands updated to use `go build -C src -o msgraphtool.exe`
...
```powershell
# Old
go build -o msgraphtool.exe

# New
go build -C src -o msgraphtool.exe
# OR
cd src && go build -o ../msgraphtool.exe
```
