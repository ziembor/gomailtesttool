...
- `src/msgraphtool_test.go` - Added `TestValidateMessageID()` with 30 test cases (lines 454-508)
...
2. **Audit:** Review CSV logs (`_msgraphtool_searchandexport_*.csv`) for suspicious Message-ID patterns
...
- This is a patch release (1.21.0 → 1.21.1) containing only security fixes
