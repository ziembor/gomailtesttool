// Package main is a backward-compatibility shim for jmaptool.
// It translates the old -action=X flag style to the new Cobra subcommand style
// and delegates to the gomailtest binary.
//
// Old: jmaptool -action=testconnect -host=jmap.example.com -port=443 ...
// New: gomailtest jmap testconnect --host=jmap.example.com --port=443 ...
//
...
func main() {
	...
	if err != nil {
		...
		fmt.Fprintln(os.Stderr, "Install gomailtest alongside jmaptool, or add it to your PATH.")
		os.Exit(1)
	}
...
// translateArgs converts old jmaptool flag-style arguments to the new Cobra
// subcommand style expected by gomailtest.
//
// Transformation:
//   - Extracts the action from -action=X or -action X (default: testconnect)
//   - Prepends ["jmap", actionName] to the remaining flags
...
func translateArgs(args []string) []string {
	...
	return append([]string{"jmap", action}, rest...)
}
