// Package main is a backward-compatibility shim for imaptool.
// It translates the old -action=X flag style to the new Cobra subcommand style
// and delegates to the gomailtest binary.
//
// Old: imaptool -action=testconnect -host=imap.example.com -port=143 ...
// New: gomailtest imap testconnect --host=imap.example.com --port=143 ...
//
...
func main() {
	...
	if err != nil {
		...
		fmt.Fprintln(os.Stderr, "Install gomailtest alongside imaptool, or add it to your PATH.")
		os.Exit(1)
	}
...
// translateArgs converts old imaptool flag-style arguments to the new Cobra
// subcommand style expected by gomailtest.
//
// Transformation:
//   - Extracts the action from -action=X or -action X (default: testconnect)
//   - Prepends ["imap", actionName] to the remaining flags
...
func translateArgs(args []string) []string {
	...
	return append([]string{"imap", action}, rest...)
}
