...
// Package main is a backward-compatibility shim for msgraphtool.
// It translates the old -action=X flag style to the new Cobra subcommand style
// and delegates to the gomailtest binary.
//
// Old: msgraphtool -action=sendmail -tenantid=... -clientid=... ...
// New: gomailtest msgraph sendmail --tenantid=... --clientid=... ...
//
...
func main() {
	...
	if err != nil {
		...
		fmt.Fprintln(os.Stderr, "Install gomailtest alongside msgraphtool, or add it to your PATH.")
		os.Exit(1)
	}
...
// translateArgs converts old msgraphtool flag-style arguments to the new Cobra
// subcommand style expected by gomailtest.
//
// Transformation:
//   - Extracts the action from -action=X or -action X (default: getinbox)
//   - Prepends ["msgraph", actionName] to the remaining flags
//   - Converts single-dash long flags to double-dash (e.g. -tenantid → --tenantid)
//   - Drops -completion and -version flags (handled by gomailtest natively)
func translateArgs(args []string) []string {
	action := ActionGetInbox // default
	var rest []string
	...
	for i := 0; i < len(args); i++ {
		...
		// Drop old -completion flag (gomailtest has Cobra's built-in completion)
		if arg == "-completion" || (len(args) > i && args[i] == "-completion") {
			if i+1 < len(args) && !strings.HasPrefix(args[i+1], "-") {
				i++ // skip the shell type argument too
			}
			continue
		}
	...
	return append([]string{"msgraph", action}, rest...)
}

// ActionGetInbox is the default action when none is specified.
const ActionGetInbox = "getinbox"
